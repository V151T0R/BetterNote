<div align="center">
  <img src="resources/icons/app_icon.png" width="128" height="128" alt="BetterNote Icon" />
  <h1>BetterNote</h1>
  <p>A minimal, distraction-free handwritten digital notebook for Windows.</p>
  <p><i>Vibe coded with care as an offline hobby project.</i></p>
</div>

---

### ✨ Highlights

- **Natural Inking**: Smooth, pressure-sensitive vector pen & blended highlighter.
- **Essential Tools**: Pen, Highlighter, Eraser, and a Lasso Selection tool to move strokes.
- **Distraction-Free**: Floating frosted toolbars, calming desk background, and clean paper.
- **Your Notes, Yours**: Saves directly to lightweight `.json` files. No accounts, no clouds, 100% offline.
- **Exports**: Quick export to PDF or images (`.png`, `.jpg`) when you need to share.

---

###  Getting Started

#### Option 1: Just Run the App
Download **`BetterNote.exe`** from [Releases]([https://github.com/](https://github.com/V151T0R/BetterNote/releases/tag/v1.0.0)]) (or run `dist/BetterNote.exe`) and double-click to launch. No installation or Python required.

#### Option 2: Run from Source
```bash
# Clone & enter folder
git clone https://github.com/your-username/BetterNote.git
cd BetterNote

# Install requirements
pip install -r requirements.txt

# Run
python main.py
```

#### Build Your Own `.exe`
Simply double-click **`build.bat`** (or run `pyinstaller BetterNote.spec --noconfirm`).

---

### ⌨️ Shortcuts

- `P` — Pen
- `H` — Highlighter
- `E` — Eraser
- `S` — Selection Tool
- `Ctrl + Z` / `Ctrl + Y` — Undo / Redo
- `Ctrl + S` — Save
- `Ctrl + O` — Open
- `Space + Drag` — Pan canvas
- `Ctrl + +` / `Ctrl + -` — Zoom in / out
- `Ctrl + 0` — Reset zoom
- `F11` — Fullscreen

---

### 🛠️ Built With

- **Python**
- **PySide6 (Qt)**
- **PyInstaller**

---

<div align="center">
  <sub>Made for peaceful scribbling and sketching. Open source under the MIT License.</sub>
</div>
