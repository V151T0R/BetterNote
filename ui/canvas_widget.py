...
class CanvasWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if isinstance(self.engine.active_tool, Selection):
            exclude_indices = self.engine.active_tool.selected_indices
        elif isinstance(self.engine.active_tool, Eraser):
            exclude_indices = list(self.engine.active_tool.strokes_to_delete)
            
        self.render_page(painter, self.engine.current_page_index, page_rect, scale=self.zoom, exclude_strokes=exclude_indices)
        ...
        if isinstance(self.engine.active_tool, (Pen, Highlighter)):
            ...
            if tool.is_erasing and tool.cursor_x is not None and tool.cursor_y is not None:
                painter.save()
                painter.translate(page_rect.topLeft())
                painter.scale(self.zoom, self.zoom)
                painter.setClipRect(0, 0, BASE_PAGE_WIDTH, BASE_PAGE_HEIGHT)
                ...
                painter.drawEllipse(QPointF(tool.cursor_x, tool.cursor_y), tool.radius, tool.radius)
                painter.restore()
